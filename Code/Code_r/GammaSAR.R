...
#
# Function that simulates GI0 deviates
#
# @param n number of observations
# @param a roughness (negative value)
# @param g scale (positive value)
# @param L number of looks (positive value, usually at least 1)
#
# It will return n observations from the GI0 model such that
# the mean is g/(a-1) and they have L looks
# 
rGI0 <- function(n, a, g, L){
  #x <- rinvgamma(n, shape=a, rate=1/g)
  x <- rinvgamma(n, shape=-a, rate=1/g)
  y <- rgamma(n, shape=L, scale=1/L)
  return(x*y)
}
