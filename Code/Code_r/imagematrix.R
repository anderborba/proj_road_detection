...
imagematrixTiff <- function(x, name){
  dimensions <- dim(x)
  zero4 <- rep(0,4)
  tiff(file=name, width=dimensions[2], height=dimensions[1])
  par(mar=zero4, oma=zero4, omi=zero4)
  plot(x)
  dev.off()
}
...
# Add Anderson Borba 
# 13 March 2025
# Function to perform ECDF-based equalization in a channel
equalize_ecdf <- function(img) {
  values <- getValues(img)
  ecdf_values <- ecdf(values)(values)
  equalized_values <- ecdf_values * maxValue(img)
  img[] <- equalized_values
  return(img)
}
### Added by Alejandro C. Frery
### 31 January 2018
